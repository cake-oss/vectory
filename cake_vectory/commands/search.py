...
def search(ctx: typer.Context):
    ...
    console.print(
        "Please specify a search type. Use [bold]vectory search --help[/bold] for more information."
    )
...
def search_by_text(
    ...
    filter_json: Optional[str] = typer.Option(
        None, "--filter", "-f", help="JSON string with filter criteria"
    ),
):
    ...
    console.print(
        f"🔍 Searching in collection [bold]{collection_name}[/bold] for: [bold]{query}[/bold]..."
    )
    ...
    try:
        ...
        if "error" in results:
            console.print(f"❌ GraphQL Error: [bold red]{results['error']}[/bold red]")
            return
...
def search_hybrid(
    ...
    alpha: float = typer.Option(
        0.5, "--alpha", "-a", help="Balance between vector and keyword search (0.0-1.0)"
    ),
    limit: int = typer.Option(
        10, "--limit", "-l", help="Maximum number of results to return"
    ),
    ...
    vector_file: Optional[str] = typer.Option(
        None,
        "--vector-file",
        help="Path to a JSON file containing a vector to use instead of generating one from the query text",
    ),
    fusion_type: str = typer.Option(
        "rankedFusion",
        "--fusion-type",
        help="Type of fusion to use (rankedFusion or relativeScoreFusion)",
    ),
    properties: Optional[str] = typer.Option(
        None,
        "--properties",
        "-p",
        help="Comma-separated list of properties to search in (e.g., 'title,description' or 'title^3,description')",
    ),
):
        ...
        try:
            with open(vector_file, "r") as f:
                ...
                if isinstance(vector_data, list):
                    ...
                    console.print(
                        f"❌ Error: Vector file must contain a JSON array of numbers"
                    )
                    return
            console.print(
                f"✅ Loaded vector with [bold]{len(vector)}[/bold] dimensions from file"
            )
        except Exception as e:
            console.print(
                f"❌ Error loading vector file: [bold red]{str(e)}[/bold red]"
            )
            return
    ...
    if fusion_type not in ["rankedFusion", "relativeScoreFusion"]:
        console.print(
            f"❌ Error: fusion_type must be either 'rankedFusion' or 'relativeScoreFusion'"
        )
        return

    console.print(
        f"🔍 Hybrid searching in collection [bold]{collection_name}[/bold] for: [bold]{query}[/bold] (alpha={alpha}, fusion={fusion_type})..."
    )
...
def search_by_filter(
    ...
    console.print(
        f"🔍 Filtering objects in collection [bold]{collection_name}[/bold]..."
    )
