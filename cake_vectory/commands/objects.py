...
@app.command("list")
def list_objects(
    collection_name: str = typer.Argument(..., help="Name of the collection to list objects from"),
    limit: int = typer.Option(10, "--limit", "-l", help="Maximum number of objects to return"),
    ...
    console.print(f"🔍 Fetching objects from collection [bold]{collection_name}[/bold] in [bold]{config['api_url']}[/bold]...")
    
    try:
        objects = api.get_objects(collection_name, limit=limit, offset=offset, tenant=tenant)
        
        if not objects or not objects.get("objects", []):
            console.print(f"ℹ️ No objects found in collection [bold]{collection_name}[/bold].")
            return
        ...
        console.print(f"📊 Found [bold]{total_count}[/bold] objects in total, showing {len(object_list)}.")
...
@app.command("get")
def get_object(
    collection_name: str = typer.Argument(..., help="Name of the collection the object belongs to"),
    ...
    console.print(f"🔍 Fetching object [bold]{object_id}[/bold] from collection [bold]{collection_name}[/bold] in [bold]{config['api_url']}[/bold]...")
    
    try:
        obj = api.get_object(collection_name, object_id, tenant=tenant)
        
        if not obj:
            console.print(f"❌ Object [bold red]{object_id}[/bold red] not found in collection [bold]{collection_name}[/bold].")
            return
...
@app.command("create")
def create_object(
    collection_name: str = typer.Argument(..., help="Name of the collection to create the object in"),
    properties_json: str = typer.Option(None, "--properties", "-p", help="JSON string of properties"),
    properties_file: Optional[str] = typer.Option(None, "--file", "-f", help="JSON file containing properties"),
    object_id: Optional[str] = typer.Option(None, "--id", help="Custom UUID for the object (generated if not provided)"),
    tenant: Optional[str] = typer.Option(None, "--tenant", "-t", help="Tenant name for multi-tenant collections")
):
    ...
    if properties_file:
        try:
            ...
            console.print(f"❌ Error reading properties file: [bold red]{str(e)}[/bold red]")
            return
    elif properties_json:
        try:
            properties = json.loads(properties_json)
        except Exception as e:
            console.print(f"❌ Error parsing properties JSON: [bold red]{str(e)}[/bold red]")
            return
    ...
    console.print(f"🔍 Creating object in collection [bold]{collection_name}[/bold] with ID [bold]{object_id}[/bold]...")
    ...
    try:
        ...
        console.print(f"✅ Object created successfully with ID: [bold green]{result.get('id', object_id)}[/bold green]")
...
def update_object(
    ...
    properties_json: str = typer.Option(None, "--properties", "-p", help="JSON string of properties to update"),
    properties_file: Optional[str] = typer.Option(None, "--file", "-f", help="JSON file containing properties to update"),
    tenant: Optional[str] = typer.Option(None, "--tenant", "-t", help="Tenant name for multi-tenant collections")
):
    ...
    console.print(f"🔍 Updating object [bold]{object_id}[/bold] in collection [bold]{collection_name}[/bold]...")
    ...
    try:
        ...
        console.print(f"✅ Object [bold green]{object_id}[/bold green] updated successfully!")
...
def delete_object(
    ...
    console.print(f"🔍 Preparing to delete object [bold]{object_id}[/bold] from collection [bold]{collection_name}[/bold]...")
    
    # Confirm deletion if not forced
    if not force:
        confirm = typer.confirm(f"Are you sure you want to delete the object '{object_id}'? This action cannot be undone.")
        if not confirm:
    ...
    try:
        ...
        console.print(f"✅ Object [bold green]{object_id}[/bold green] deleted successfully!")
...
def search_objects(
    ...
    query_text: str = typer.Option(None, "--text", "-t", help="Text to search for (will be vectorized)"),
    limit: int = typer.Option(10, "--limit", "-l", help="Maximum number of results to return"),
    tenant: Optional[str] = typer.Option(None, "--tenant", help="Tenant name for multi-tenant collections"),
    filter_json: Optional[str] = typer.Option(None, "--filter", help="JSON string with filter criteria")
):
    ...
    console.print(f"🔍 Searching in collection [bold]{collection_name}[/bold] for: [bold]{query_text}[/bold]...")
...
@app.command("count")
def count_objects(
    collection_name: str = typer.Argument(..., help="Name of the collection to count objects from"),
    tenant: Optional[str] = typer.Option(None, "--tenant", "-t", help="Tenant name for multi-tenant collections")
):
    ...
    console.print(f"🔍 Counting objects in collection [bold]{collection_name}[/bold] in [bold]{config['api_url']}[/bold]...")
    
    try:
        # Get the total count
        count = api.get_collection_count(collection_name, tenant=tenant)
        
        console.print(f"📊 Collection [bold]{collection_name}[/bold] contains [bold green]{count}[/bold green] objects.")
...
@app.command("batch")
def batch_objects(
    collection_name: str = typer.Argument(..., help="Name of the collection to create objects in"),
    input_file: str = typer.Argument(..., help="JSON file containing array of objects to create"),
    tenant: Optional[str] = typer.Option(None, "--tenant", "-t", help="Tenant name for multi-tenant collections")
):
    ...
    try:
        ...
        console.print(f"📦 Preparing to create [bold]{len(objects_data)}[/bold] objects in collection [bold]{collection_name}[/bold]...")
        ...
        console.print(f"✅ Batch operation completed: [bold green]{success_count}[/bold green] objects created successfully, [bold red]{failure_count}[/bold red] failed.")
        ...
        if failure_count > 0:
            ...
            for result in results:
                if result.get("result", {}).get("status") != "SUCCESS":
                    ...
                    console.print(f"   Error: {result.get('result', {}).get('errors', 'Unknown error')}")
