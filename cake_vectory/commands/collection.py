...
@app.command()
def info(
    collection_name: str = typer.Argument(..., help="Name of the collection to get info for")
):
    """Get detailed information about a collection."""
    api = WeaviateAPI()
    config = get_config()
    
    console.print(f"🔍 Fetching info for collection [bold]{collection_name}[/bold] from [bold]{config['api_url']}[/bold]...")
    ...
    try:
        ...
        if not schema:
            console.print(f"❌ Collection [bold red]{collection_name}[/bold red] not found.")
            return
        ...
        panel_content = f"[bold cyan]Collection:[/bold cyan] {schema.get('class')}\n" \
        ...
        console.print(f"❌ Error fetching collection info: [bold red]{str(e)}[/bold red]")
...
def delete(
    ...
    console.print(f"🔍 Preparing to delete collection [bold]{collection_name}[/bold] from [bold]{config['api_url']}[/bold]...")
    ...
    try:
        ...
        try:
            stats = api.get_collection_stats(collection_name)
            object_count = stats.get("object_count", 0)
            console.print(f"ℹ️ Collection [bold]{collection_name}[/bold] contains [bold yellow]{object_count}[/bold yellow] objects.")
        except Exception:
            console.print(f"⚠️ Could not determine object count for [bold]{collection_name}[/bold].")
        
        # Confirm deletion if not forced
        if not force:
            confirm = typer.confirm(f"Are you sure you want to delete the collection '{collection_name}'? This action cannot be undone.")
            if not confirm:
        ...
        console.print(f"✅ Collection [bold green]{collection_name}[/bold green] deleted successfully!")
...
def shards_info(
    collection_name: str = typer.Argument(..., help="Name of the collection to get shard info for"),
    detailed: bool = typer.Option(False, "--detailed", "-d", help="Show detailed information about each shard"),
    shard_name: str = typer.Option(None, "--shard", "-s", help="Show detailed information for a specific shard only")
):
    """Get detailed information about a collection's shards."""
    api = WeaviateAPI()
    config = get_config()
    
    console.print(f"🔍 Fetching shard information for collection [bold]{collection_name}[/bold] from [bold]{config['api_url']}[/bold]...")
    ...
    try:
        ...
        if not shards:
            console.print(f"ℹ️ No shard information available for collection [bold]{collection_name}[/bold].")
            return
            
        # Display shards in a table
        table = Table(title=f"Shards for Collection: {collection_name}", border_style="cyan")
        ...
        for shard_name, shard_info in shard_items:
            ...
            objects = int(objects_str) if isinstance(objects_str, str) and objects_str.isdigit() else objects_str
        ...
        console.print(f"📊 Found [bold]{shard_count}[/bold] shards with a total of [bold]{total_objects}[/bold] objects.")
        console.print(table)
        ...
        if shard_name:
            ...
            if shard_name in shards:
                ...
                console.print(f"❌ Shard [bold red]{shard_name}[/bold red] not found in collection [bold]{collection_name}[/bold].")
                return
        elif detailed:
        ...
        for shard_name, shard_info in detailed_shards:
            ...
            # Basic shard info
            status = shard_info.get("status", "Unknown")
            status_color = "green" if status == "READY" else "yellow" if status in ["CREATING", "PENDING"] else "red" if status in ["ERROR", "FAILED"] else "white"
            panel_content += f"[bold blue]Status:[/bold blue] [{status_color}]{status}[/{status_color}]\n"
            panel_content += f"[bold blue]Node:[/bold blue] {shard_info.get('node', 'Unknown')}\n"
            ...
            if "vectorIndexingStatus" in shard_info:
                ...
                vector_color = "green" if vector_status == "READY" else "yellow" if vector_status in ["BUILDING", "PENDING"] else "red" if vector_status in ["ERROR", "FAILED"] else "white"
            ...
            if "indexingProgress" in shard_info:
                ...
                panel_content += f"[bold blue]Indexing Progress:[/bold blue] {index_progress}\n"
            
            # Add performance metrics if available
            if "queryTimeAvg" in shard_info:
                query_time = shard_info.get("queryTimeAvg", "Unknown")
                panel_content += f"[bold blue]Avg. Query Time:[/bold blue] {query_time}\n"
            ...
            if "replicas" in shard_info and shard_info.get("replicas"):
                ...
                for i, replica in enumerate(replicas):
                    ...
                    r_status_color = "green" if r_status == "READY" else "yellow" if r_status in ["CREATING", "PENDING"] else "red" if r_status in ["ERROR", "FAILED"] else "white"
                    ...
                    if "health" in replica:
                        ...
                        panel_content += f"    Health: [{health_color}]{health}[/{health_color}]\n"
                    
                    # Add replica sync status if available
                    if "syncStatus" in replica:
                        sync = replica.get("syncStatus", "Unknown")
                        sync_color = "green" if sync == "IN_SYNC" else "yellow" if sync == "SYNCING" else "red"
                        panel_content += f"    Sync Status: [{sync_color}]{sync}[/{sync_color}]\n"
        ...
        console.print(f"❌ Error fetching shard information: [bold red]{str(e)}[/bold red]")


@app.command("replication")
def replication_info(
    collection_name: str = typer.Argument(..., help="Name of the collection to get replication info for")
):
    """Get replication information about a collection."""
    api = WeaviateAPI()
    config = get_config()
    
    console.print(f"🔍 Fetching replication information for collection [bold]{collection_name}[/bold] from [bold]{config['api_url']}[/bold]...")
    ...
    try:
        ...
        if not replication_config:
            console.print(f"ℹ️ No replication information available for collection [bold]{collection_name}[/bold].")
            return
        ...
        if shards:
            ...
            for shard_name, shard_info in shards.items():
                is_replicated = "replicas" in shard_info and len(shard_info.get("replicas", [])) > 0
        ...
        console.print(f"❌ Error fetching replication information: [bold red]{str(e)}[/bold red]")
