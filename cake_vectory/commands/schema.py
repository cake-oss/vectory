...
@app.command()
def get(
    collection_name: str = typer.Argument(..., help="Name of the collection schema to get")
):
    """Get details of a specific collection schema."""
    api = WeaviateAPI()
    config = get_config()
    
    console.print(f"🔍 Fetching schema for collection [bold]{collection_name}[/bold] from [bold]{config['api_url']}[/bold]...")
    ...
    try:
        ...
        if not schema:
            console.print(f"❌ Collection schema [bold red]{collection_name}[/bold red] not found.")
            return
...
@app.command()
def delete(
    collection_name: str = typer.Argument(..., help="Name of the collection schema to delete"),
